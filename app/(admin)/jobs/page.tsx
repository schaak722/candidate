      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div className="w-full max-w-3xl overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-xl">
          <div className="flex items-center justify-between border-b border-zinc-200 px-5 py-4">
            ...
            <button onClick={onClose} className="rounded-lg p-2 text-zinc-500 hover:bg-zinc-100" aria-label="Close">
              <X className="h-4 w-4" />
            </button>
          </div>
...
export default function JobsPage() {
  ...
  // read URL params client-side (no useSearchParams => no Suspense build error)
  useEffect(() => {
    ...
    menu && typeof document !== "undefined"
      ? createPortal(
          <div className="fixed inset-0 z-40" onClick={() => setMenu(null)}>
            ...
            >
              <button className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-zinc-800 hover:bg-zinc-50" onClick={() => openEdit(menu.id)}>
                <Pencil className="h-4 w-4" />
                Edit
              </button>
              <button className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm text-red-700 hover:bg-red-50" onClick={() => onDelete(menu.id)}>
    ...
    <div className="min-h-screen">
            ...
            <Table>
              <thead className="bg-zinc-50 text-left text-sm font-semibold text-zinc-700">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right" aria-label="Actions">&nbsp;</th>
                </tr>
              </thead>
              <tbody>
                {jobs.map((j) => (
                  <tr key={j.id} className="border-t border-zinc-200">
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-zinc-900">
                      <button type="button" className="text-left hover:text-zinc-700" onClick={() => openView(j.id)}>
                        {j.title}
                      </button>
                    </td>
      ...
      {viewingId ? (
        <ModalShell title="Job Details" onClose={() => setViewingId(null)}>
          {viewError ? <div className="mb-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{viewError}</div> : null}
          {viewLoading ? (
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-zinc-200 p-4">
                ...
                <div className="mt-4">
                  ...
                  <div className="prose prose-sm mt-2 max-w-none rounded-xl border border-zinc-200 bg-white p-3" dangerouslySetInnerHTML={{ __html: safeHtml(viewJob.description || "") || "<em>No description.</em>" }} />
                </div>
              </div>
            </div>
          ) : null}
      ...
      {/* EDIT MODAL (reordered to match your screenshot) */}
      {editId ? (
        <ModalShell
          title="Edit Job"
        ...
        >
          {editError ? <div className="mb-4 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{editError}</div> : null}
              ...
              <div className="rounded-xl border border-zinc-200 p-4">
                ...
                <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div>
                    ...
                    <Input value={editForm.title} onChange={(e) => setEditForm((f) => ({ ...f, title: e.target.value }))} className={issueFor("title") ? "border-red-300" : ""} />
                    {issueFor("title") ? <div className="mt-1 text-xs text-red-600">{issueFor("title")}</div> : null}
                  </div>

                  <div>
                    <Label>Job Ref ID *</Label>
                    <Input value={editForm.refId} onChange={(e) => setEditForm((f) => ({ ...f, refId: e.target.value }))} className={issueFor("refId") ? "border-red-300" : ""} />
                  ...
                  <div>
                    <Label>Basis *</Label>
                    <Input value={editForm.basis} onChange={(e) => setEditForm((f) => ({ ...f, basis: e.target.value }))} className={issueFor("basis") ? "border-red-300" : ""} />
                  ...
                  <div>
                    ...
                    <Select value={editForm.seniority} onChange={(e) => setEditForm((f) => ({ ...f, seniority: e.target.value as any }))}>
